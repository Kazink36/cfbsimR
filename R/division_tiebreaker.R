...
break_division_ties <- function(u, r, h2h, tb_depth, .debug = FALSE) {
  ...
  if (any(is.na(u$div_rank)) && tb_depth > TIEBREAKERS_NONE) {
    ...
    for (min_tied in 3:2)
    {
      ...
      list[u, tied] <- tied %>%
        inner_join(tied %>% select(sim, conf, division, team, win_pct),
        ...
        group_by(sim, conf, division, team, div_pct, conf_pct, sov, sos, tied_teams) %>%
      ...
      # record against highest ranked in division NEED TO IMPLEMENT
      if (isTRUE(.debug)) report(glue("DIV ({min_tied}): Common Record"))
      list[u, tied] <- tied %>%
        inner_join(h2h, by = c("sim", "team")) %>%
        filter(h2h_played == 1) %>%
        group_by(sim, conf, division, opp) %>%
      ...
      # record against other division NEED TO IMPLEMENT
...
process_div_ties <- function(t, u, r) {
  ...
  # 0 = teams eliminated from tiebreaker
  t <- t %>%
    group_by(sim, conf, division) %>%
