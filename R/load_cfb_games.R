...
load_cfb_games <- function(season){
  ...
  #     glue::glue("https://github.com/sportsdataverse/cfbfastR-data/blob/main/schedules/rds/cfb_schedules_{season}.rds?raw=true")
  #   ),
  #   silent = TRUE)
  #if (inherits(fetched, "try-error") || fetched$status_code != 200) return(tibble::tibble())
  #orig_df <- cfbfastR:::read_raw_rds(fetched$content)
  ...
  rest_df <- orig_df %>%
    mutate(sim = game_id) %>%
    cfbsimR:::double_games() %>%
    left_join(orig_df %>% select(game_id,start_date,season),
              by = c("sim" = "game_id")) %>%
    mutate(start_date = as.Date(as.POSIXct(start_date)-7*60*60)) %>%
    arrange(start_date) %>%
  ...
  orig_df %>%
    ...
    mutate(
      fcs_h = ifelse(home_team %in% (divisions %>%
                                       filter(team!="FCS") %>%
                                       pull(team)),0,1),

      fcs_a = ifelse(away_team %in% (divisions %>%
                                       filter(team!="FCS") %>%
                                       pull(team)),0,1)
      ) %>%
