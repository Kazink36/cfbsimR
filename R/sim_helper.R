...
#FIGURE OUT HOW THIS APPLIES
  ...
  for (week_num in weeks_to_sim) {
    return_value <-
      ...
      #simulate_week(teams, games, week_num,test_week = NULL, elo_df = elo_df,elo_model = elo_model)
    if (!is.null(test_week) && week_num == test_week) {
      return(return_value)
    }
    teams <- return_value$teams
  ...
  #### FIND DIVISIONAL STANDINGS AND CONFERENCE CHAMPIONSHIPS ####
  ...
  # conferences with a championship
  championship_confs <- c(
    "AAC","ACC","B1G","CUSA","MAC","MWC","SEC"
  )

  # identify division champs with a conference championship
  division_champs <- teams %>%
    filter((div_rank == 1 & conf %in% championship_confs) |
            (conf %in% c("B12","PAC") & div_rank <= 2)) %>%
    select(sim, conf, division, team) %>%
    arrange(sim, conf, division)

  # conference championship week
  conf_champ_week <- week_num + 1

  add_games <- division_champs %>%
    group_by(sim, conf) %>%
    summarize(season = first(schedule$season),
              game_type = "CONF",
              week = conf_champ_week,
              away_team = first(team),
              home_team = last(team),
              away_rest = 7,
              home_rest = 7,
              location = "Neutral") %>%
  ...
  return_value <-
    simulate_week(teams, games, conf_champ_week, test_week, ...)
  if (!is.null(test_week) && conf_champ_week == test_week) {
    return(return_value)
  }
  teams <- return_value$teams
  games <- return_value$games

  # record conf champ results
  teams <- games %>%
    filter(week == conf_champ_week) %>%
    double_games() %>%
    #filter(outcome == 0) %>%
    select(sim, team, outcome) %>%
    right_join(teams, by = c("sim", "team")) %>%
    mutate(games = ifelse(!is.na(outcome), games + 1, games),
           wins = ifelse(!is.na(outcome), wins + outcome, wins),
           conf_champ = case_when(outcome == 1 ~ 1,
                                  #!(conf %in% championship_confs) & div_rank == 1 ~ 1,
                                  TRUE ~ 0),
           win_pct = wins/games
           ) %>%
    select(-outcome,)
