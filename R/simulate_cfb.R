...
#' @param cfb_season Season to simulate
...
#' ```
#' simulate_cfb(2020, fresh_season = TRUE) %>%
...
#' dplyr::glimpse(sim)
#' }
simulate_cfb <- function(cfb_season = NULL,
                         process_games = NULL,
                         ...,
                         playoff_seeds = 4,
  ...
  if (is.null(process_games)) {
    process_games <- function(teams, games, week_num, ...) {
      ...
      teams <- teams %>%
        ...
        group_by(sim, team) %>%
        mutate(elo_shift = sum(elo_shift)) %>%
        ungroup() %>%
  ...
  if (!all(
    is.null(cfb_season) || is_single_digit_numeric(cfb_season),
    ...
    stop(
      "One or more of the parameters `cfb_season`, `tiebreaker_depth`, `test_week`, ",
  ...
  if (is.null(cfb_season)) {
    # Come up with a way to get max season available
    cfb_season <- 2023 #max(schedule$season)
  }

  if (cfb_season < 2001) {
    stop("The earliest season that can be simulated is 2001.")
  }

  if (cfb_season > 2023) {
    stop("The most recent season that can be simulated is 2023.")
  }
  ...
  schedule <- load_cfb_games(cfb_season) %>%
    select(
      season, game_type, game_id, week, away_team, home_team,
      away_rest, home_rest, location, result
    )

  # Don't currently have a way of implementing this for CFB
  # if (nrow(schedule) == 0)
  # {
  #   fn <- glue::glue("https://github.com/nflverse/nfldata/blob/master/fake_schedule_{cfb_season}.csv?raw=true")
  #   tryCatch({
  #     options(readr.num_columns = 0)
  #     schedule <- readr::read_csv(fn)
  #     sim_info(glue::glue("No actual schedule exists for {cfb_season}, using fake schedule with correct opponents"))
  #   }, error = function(cond) {
  #     stop("Unable to locate a schedule for ", cfb_season)
  #   })
  # }
  ...
  report(glue::glue("Beginning simulation of {simulations} seasons in {sim_rounds} {ifelse(sim_rounds == 1, 'round', 'rounds')}"))
  ...
  championship_confs <- c(
    "AAC","ACC","B1G","CUSA","MAC","MWC","SEC"
  )
  overall <- all_teams %>%
    group_by(conf, division, team) %>%
    summarize(
      bowl = mean(wins >= 6),
      wins_ten = mean(wins >= 10),
      wins = mean(wins),
      #playoff = mean(!is.na(seed)),
      div1 = mean(ifelse((div_rank == 1 & conf %in% championship_confs) |
                           (conf %in% c("B12","PAC") & div_rank <= 2),1,0)),
      #seed1 = mean(!is.na(seed) & seed == 1),
      won_conf = mean(conf_champ)
    ...
    ungroup() %>%
    select(conf,division,team,wins,dplyr::everything())
